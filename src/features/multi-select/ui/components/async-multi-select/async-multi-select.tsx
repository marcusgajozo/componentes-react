...
import { MultiSelect, type MultiSelectProps, type Option } from "../multi-select/multi-select";

export type AsyncMultiSelectProps<TResult, TItem> = {
  ...
  debounceMs?: number;
} & Omit<MultiSelectProps, "options" | "onSearchChange" | "isLoading">;

export function AsyncMultiSelect<TResult, TItem>({
  loadOptions,
  getArray,
  getLabel,
  getValue,
  ...
  const optionsToPass = React.useMemo(() => {
    ...
    const missingValues = selectedValues.filter((val) => !options.some((o) => o.value === val));
    if (missingValues.length === 0) return options;

    const additionalOptions = missingValues
      .map((val) => cacheByValue.get(val))
      .filter((opt): opt is Option => opt !== undefined);

    return [...options, ...additionalOptions];
  }, [cacheByParams, debouncedTerm, selectedValues, cacheByValue]);
